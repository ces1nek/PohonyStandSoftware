...
void stand_im_periodic_isr(void) {
	...
	hhrtim1.Instance->sTimerxRegs[0].CMP1xR = relUtoCmp(
			*x2cModel.outports.bOutPWMa);
	hhrtim1.Instance->sTimerxRegs[1].CMP1xR = relUtoCmp(
			*x2cModel.outports.bOutPWMb);
	hhrtim1.Instance->sTimerxRegs[2].CMP1xR = relUtoCmp(
